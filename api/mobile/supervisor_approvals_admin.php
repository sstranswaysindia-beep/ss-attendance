...
$allowedUsers = ['vikas_sachan', 'neeraj', 'dhirendra1975'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['attendance_id'], $_POST['action'])) {
    ...
    if (!hash_equals($csrfToken, $token)) {
        if ($isAjax)
            $sendJson('error', 'Invalid or expired form token. Please refresh the page.');
        supervisorRedirectWithFlash('danger', 'Invalid or expired form token. Please try again.');
    }

    if ($currentUserId <= 0) {
        if ($isAjax)
            $sendJson('error', 'Your admin session is missing a user id. Please log in again.');
    ...
    if (!$attendanceId || $attendanceId <= 0) {
        if ($isAjax)
            $sendJson('error', 'Invalid attendance id.');
        supervisorRedirectWithFlash('danger', 'Invalid attendance id.');
    }
    if (!in_array($action, ['approve', 'reject'], true)) {
        if ($isAjax)
            $sendJson('error', 'Unsupported action requested.');
    ...
    try {
        ...
        if (!$attendanceRow) {
            if ($isAjax)
                $sendJson('error', 'Attendance record not found.');
...
if (!in_array($roleRaw, $roleOptions, true))
    $roleRaw = 'supervisor';
...
<head>
    ...
    <style>
        ...
        body>.container-fluid.page-wrapper {
            padding-left: 0.5rem;
        ...
        @media (max-width: 767.98px) {
            ...
            .mobile-card-table tr[data-row-type="approval"] {
                ...
                /* Hide original rows on mobile */
            }

            .approvals-card .card-body {
                padding: 0.5rem;
            }

            .approvals-card .table-responsive {
                border-radius: 14px;
                background: transparent;
            }
            ...
            .swipe-card-wrapper.swiping-left .swipe-bg-approve {
                opacity: 1;
            }

            .swipe-card-wrapper.swiping-right .swipe-bg-reject {
                opacity: 1;
            }
            ...
            .swipe-card {
                position: relative;
                z-index: 2;
                background: linear-gradient(145deg, #fffef5 0%, #fef9e7 100%);
                border-radius: 20px;
                padding: 16px 16px 14px;
                box-shadow: 0 6px 24px rgba(15, 23, 42, 0.08), 0 1.5px 6px rgba(15, 23, 42, 0.04);
            ...
            .swipe-card.fly-left {
                ...
                transform: translateX(-120vw) rotate(-8deg) !important;
                opacity: 0;
            }
            ...
            .sc-header .avatar {
                ...
                flex-shrink: 0;
                font-size: .95rem;
                box-shadow: 0 3px 10px rgba(13, 71, 161, 0.13);
            }
            ...
            /* ---- Pending checkout state (light red) ---- */
            .swipe-card.no-checkout {
                background: linear-gradient(145deg, #fff5f5 0%, #fee2e2 100%);
                border: 1px solid rgba(248, 113, 113, 0.3);
                border-left: 4px solid #f87171;
            }

            .sc-pending-checkout-tag {
                display: inline-flex;
                align-items: center;
                gap: 4px;
                background: #fee2e2;
            ...
            /* ---- Approved cards (light green) ---- */
            .swipe-card-wrapper[data-status="approved"] .swipe-card {
                background: linear-gradient(145deg, #f0fdf4 0%, #dcfce7 100%);
                border: 1px solid rgba(34, 197, 94, 0.25);
            }

            .swipe-card-wrapper[data-status="approved"] .sc-field-label {
                color: #4ade80;
            }

            /* ---- Rejected cards (dark red) ---- */
            .swipe-card-wrapper[data-status="rejected"] .swipe-card {
                background: linear-gradient(145deg, #7f1d1d 0%, #991b1b 100%);
                border: 1px solid rgba(239, 68, 68, 0.4);
            }

            .swipe-card-wrapper[data-status="rejected"] .sc-name,
            .swipe-card-wrapper[data-status="rejected"] .sc-field-value {
                color: #fef2f2;
            }

            .swipe-card-wrapper[data-status="rejected"] .sc-meta,
            .swipe-card-wrapper[data-status="rejected"] .sc-field-label {
                color: #fca5a5;
            }

            .swipe-card-wrapper[data-status="rejected"] .sc-header {
                border-bottom-color: rgba(252, 165, 165, 0.2);
            }

            .swipe-card-wrapper[data-status="rejected"] .sc-footer {
                border-top-color: rgba(252, 165, 165, 0.2);
            }

            .swipe-card-wrapper[data-status="rejected"] .sc-notes {
                background: rgba(127, 29, 29, 0.4);
                color: #fecaca;
            }

            .swipe-card-wrapper[data-status="rejected"] .sc-notes-label {
                color: #fca5a5;
            }

            .swipe-card-wrapper[data-status="rejected"] .sc-swipe-hint {
                color: #fca5a5;
            }

            .swipe-card-wrapper[data-status="rejected"] .sc-photos .btn {
                color: #fecaca;
                border-color: rgba(254, 202, 202, 0.4);
            }

            /* ---- Already resolved cards ---- */
            .swipe-card.resolved {
                opacity: .82;
            }
            ...
            .sc-action-result.action-approve {
                color: #16a34a;
            }

            .sc-action-result.action-reject {
                color: #dc2626;
            }
        ...
        /* Desktop: hide swipe cards container */
        @media (min-width: 768px) {
            .swipe-cards-container {
                display: none !important;
            }
        ...
        .approvals-card .table> :not(caption)>*>* {
            padding-top: 0.35rem;
        ...
        .missing-card .table> :not(caption)>*>* {
            padding: .28rem .45rem;
        }
        ...
        /* ---- Stats: animated moving gradients ---- */
        ...
        .stats-pending {
            background: linear-gradient(135deg, #fb923c, #f97316, #facc15, #fb923c) !important;
            background-size: 200% 200% !important;
            animation: gradient-shift 3s ease infinite;
        }

        .stats-approved {
            background: linear-gradient(135deg, #22c55e, #16a34a, #0f766e, #22c55e) !important;
            background-size: 200% 200% !important;
            animation: gradient-shift 3.5s ease infinite;
        }

        .stats-rejected {
            background: linear-gradient(135deg, #ef4444, #f97316, #f59e0b, #ef4444) !important;
            background-size: 200% 200% !important;
            animation: gradient-shift 4s ease infinite;
        }

        /* ---- Swipe hint arrows bounce ---- */
        ...
        .sc-swipe-hint .fa-arrow-left {
            animation: bounce-left 1s ease-in-out infinite;
            display: inline-block;
        }

        .sc-swipe-hint .fa-arrow-right {
        ...
        .photo-lightbox-overlay.active img {
            transform: scale(1);
        }
...
<body>
            ...
            <div class="row">
                ...
                <main class="main col-md-9 col-lg-10">
                    <div class="container-fluid py-1">
                        ...
                        <div class="pt-2 pb-2 mb-3 border rounded header-blue">
                            <div class="d-flex flex-wrap align-items-center gap-2 px-2">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <h1 class="h6 mb-0 text-uppercase text-muted" style="letter-spacing:.06em;">
                                        Attendance Approvals</h1>
                                </div>
                                ...
                                <form method="get" id="filters-form"
                                    ...
                                    <div class="role-toggle">
                                        <button type="button"
                                            class="role-pill <?= $roleFilterServer === 'driver' ? 'active' : '' ?>"
                                            data-role="driver">Drivers</button>
                                        <button type="button"
                                            class="role-pill <?= $roleFilterServer === 'helper' ? 'active' : '' ?>"
                                            data-role="helper">Helpers</button>
                                        <button type="button"
                                            class="role-pill <?= $roleFilterServer === 'supervisor' ? 'active' : '' ?>"
                                            data-role="supervisor">Supervisors</button>
                                        <button type="button"
                                            class="role-pill <?= $roleFilterServer === 'all' ? 'active' : '' ?>"
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline-success" id="currentMonthBtn"
                                        title="Current month">
                        ...
                        <div class="card approvals-card mb-4">
                            <div
                                ...
                                <div class="w-100 w-md-50">
                                    <input type="text" id="table-search-input" name="search"
                                        ...
                                        <table class="table align-middle mb-0 mobile-card-table">
                                            ...
                                            <tbody id="approvals-tbody">
                                                <?php foreach ($approvals as $approval): ?>
                                                    ...
                                                    if (!in_array($statusNorm, ['Pending', 'Approved', 'Rejected'], true))
                                                        $statusNorm = 'Pending';
                                                    ...
                                                    ?>
                                                    <tr data-row-type="approval" data-status="<?= h(strtolower($statusNorm)) ?>"
                                                        ...
                                                        <td data-label="Person">
                                                            <div class="d-flex align-items-center gap-3">
                                                                ...
                                                                <?php else: ?>
                                                                    <span
                                                                        class="avatar"><?= h(strtoupper(mb_substr((string) $driverName, 0, 1))) ?></span>
                                                                <?php endif; ?>
                                                                <div>
                                                                    <div class="fw-semibold"><?= h($driverName) ?></div>
                                                                    <div class="text-muted small">
                                                                        #<?= h((string) $approval['driverId']) ?> ·
                                                                        <?= h($roleLabel) ?>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <td data-label="In / Out"
                                                            class="<?= $differentDates ? 'text-danger fw-semibold' : '' ?>">
                                                        ...
                                                        <td data-label="Actions">
                                                            ...
                                                            <?php else: ?>
                                                                <form method="post"
                                                                    class="d-flex flex-column flex-lg-row gap-2 align-items-stretch"
                                                                    data-approval-form="1">
                                                                    <input type="hidden" name="attendance_id"
                                                                        value="<?= h((string) $approval['attendanceId']) ?>">
                        ...
                        <!-- ================= MOBILE SWIPE CARDS (only visible on mobile) ================= -->
                        <?php if (!empty($approvals)): ?>
                            <div class="swipe-cards-container" id="swipe-cards-container">
                                <?php foreach ($approvals as $approval): ?>
                                    <?php
                                    $sc_roleLabel = (string) ($approval['role'] ?? 'Driver');
                                    $sc_roleSlug = strtolower($sc_roleLabel);
                                    $sc_plantId = isset($approval['plantId']) ? (int) $approval['plantId'] : 0;
                                    $sc_driverName = (string) ($approval['driverName'] ?? '');
                                    $sc_plantName = (string) ($approval['plantName'] ?? '');
                                    $sc_notesRaw = (string) ($approval['notes'] ?? '');
                                    $sc_statusNorm = ucfirst(strtolower(trim((string) ($approval['status'] ?? 'Pending'))));
                                    if (!in_array($sc_statusNorm, ['Pending', 'Approved', 'Rejected'], true))
                                        $sc_statusNorm = 'Pending';
                                    $sc_hasCheckout = !empty($approval['outTime']);
                                    $sc_workingHours = formatWorkingHours($approval['inTime'] ?? null, $approval['outTime'] ?? null);
                                    $sc_differentDates = isDifferentInOutDate($approval['inTime'] ?? null, $approval['outTime'] ?? null);
                                    $sc_vehicleText = (string) ($approval['vehicleNumber'] ?? '');
                                    $sc_isPending = strtolower($sc_statusNorm) === 'pending';
                                    $sc_canSwipe = $sc_hasCheckout && $sc_isPending;
                                    $sc_attendanceDate = '';
                                    if (!empty($approval['inTime'])) {
                                        try {
                                            $sc_attendanceDate = (new DateTime($approval['inTime']))->format('Y-m-d');
                                        } catch (Throwable $e) {
                                        }
                                    }
                                    ?>
                                    <div class="swipe-card-wrapper"
                                        data-attendance-id="<?= h((string) $approval['attendanceId']) ?>"
                                        data-role="<?= h($sc_roleSlug) ?>" data-plant-id="<?= h((string) $sc_plantId) ?>"
                                        data-name="<?= h(mb_strtolower($sc_driverName)) ?>"
                                        data-plant="<?= h(mb_strtolower($sc_plantName)) ?>"
                                        data-vehicle="<?= h(mb_strtolower($sc_vehicleText)) ?>"
                                        data-notes="<?= h(mb_strtolower($sc_notesRaw)) ?>"
                                        data-status-label="<?= h(strtolower($sc_statusNorm)) ?>"
                                        data-status="<?= h(strtolower($sc_statusNorm)) ?>"
                                        data-date="<?= h($sc_attendanceDate) ?>" data-row-type="approval-card"
                                        data-can-swipe="<?= $sc_canSwipe ? '1' : '0' ?>">

                                        <!-- Approve bg (left swipe) -->
                                        <div class="swipe-bg swipe-bg-approve">
                                            <span>Approve</span>
                                            <i class="fas fa-check-circle"></i>
                                        </div>
                                        <!-- Reject bg (right swipe) -->
                                        <div class="swipe-bg swipe-bg-reject">
                                            <i class="fas fa-times-circle"></i>
                                            <span>Reject</span>
                                        </div>

                                        <div
                                            class="swipe-card <?= !$sc_hasCheckout ? 'no-checkout' : '' ?> <?= !$sc_isPending ? 'resolved' : '' ?>">
                                            ...
                                            <div class="sc-header">
                                                ...
                                                <?php else: ?>
                                                    <span
                                                        class="avatar"><?= h(strtoupper(mb_substr($sc_driverName, 0, 1))) ?></span>
                                                <?php endif; ?>
                                                <div class="sc-header-info">
                                                    <div class="sc-name"><?= h($sc_driverName) ?></div>
                                                    <div class="sc-meta">
                                                        #<?= h((string) $approval['driverId']) ?> · <?= h($sc_roleLabel) ?>
                                                        · <?= h(strtoupper($approval['source'] ?? 'mobile')) ?>
                                                    </div>
                                                </div>
                                                <div class="sc-status-badge">
                                                    <span
                                                        class="badge text-bg-<?= statusBadgeClass($sc_statusNorm) ?>"><?= h($sc_statusNorm) ?></span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="sc-body">
                                                <div class="sc-field">
                                                    <div class="sc-field-label">Plant</div>
                                                    <div class="sc-field-value"><?= h($sc_plantName) ?></div>
                                                </div>
                                                <div class="sc-field">
                                                    <div class="sc-field-label">Vehicle</div>
                                                    <div class="sc-field-value">
                                                        <?= h($sc_vehicleText !== '' ? $sc_vehicleText : 'TBD') ?>
                                                    </div>
                                                </div>
                                                <div class="sc-field">
                                                    <div class="sc-field-label">Check In</div>
                                                    <div class="sc-field-value <?= $sc_differentDates ? 'text-danger' : '' ?>">
                                                        <?= h(formatDateTime($approval['inTime'])) ?>
                                                    </div>
                                                </div>
                                                <div class="sc-field">
                                                    <div class="sc-field-label">Check Out</div>
                                                    <div class="sc-field-value <?= $sc_differentDates ? 'text-danger' : '' ?>">
                                                        <?= h($approval['outTime'] ? formatDateTime($approval['outTime']) : '—') ?>
                                                    </div>
                                                </div>
                                                <div class="sc-field">
                                                    <div class="sc-field-label">Working Hours</div>
                                                    <div class="sc-field-value"><?= h($sc_workingHours) ?></div>
                                                </div>
                                                <div class="sc-field">
                                                    <div class="sc-field-label">Date</div>
                                                    <div class="sc-field-value"><?= h($sc_attendanceDate ?: '—') ?></div>
                                                </div>

                                                <?php if ($sc_notesRaw !== ''): ?>
                                                    <div class="sc-notes">
                                                        <div class="sc-notes-label">Notes</div>
                                                        <?= nl2br(h($sc_notesRaw)) ?>
                                                    </div>
                                                <?php endif; ?>
                                            </div>
                                            ...
                                            <!-- Footer -->
                                            <div class="sc-footer">
                                                <?php if (!$sc_hasCheckout): ?>
                                                    <span class="sc-pending-checkout-tag">
                                                        <i class="fas fa-clock"></i> Pending checkout
                                                    </span>
                                                <?php elseif ($sc_isPending): ?>
                                                    <div class="sc-swipe-hint">
                                                        <i class="fas fa-arrow-left" style="color:#22c55e"></i>
                                                        Approve
                                                ...
                                                <?php else: ?>
                                                    <span class="text-muted" style="font-size:.72rem;">Already
                                                        <?= h($sc_statusNorm) ?></span>
                                                <?php endif; ?>
                                                ...
                                                <div class="sc-photos">
                                                    <?php if (!empty($approval['inPhotoUrl'])): ?>
                                                        <a class="btn btn-sm btn-outline-primary"
                                                            href="<?= h($approval['inPhotoUrl']) ?>"
                                                            data-photo-url="<?= h($approval['inPhotoUrl']) ?>">IN 📸</a>
                                                    <?php endif; ?>
                                                    <?php if (!empty($approval['outPhotoUrl'])): ?>
                                                        <a class="btn btn-sm btn-outline-primary"
                                                            href="<?= h($approval['outPhotoUrl']) ?>"
                                                            data-photo-url="<?= h($approval['outPhotoUrl']) ?>">OUT 📸</a>
                                                    <?php endif; ?>
                                                </div>
                                            </div>

                                            <!-- Hidden form for AJAX -->
                                            <form class="d-none" data-swipe-form="1">
                                                <input type="hidden" name="attendance_id"
                                                    value="<?= h((string) $approval['attendanceId']) ?>">
                            ...
                            <div class="card-header bg-white">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <div class="text-muted small">
                                            Based on
                                            <?= h($rangeLabel) ?><?php if ($selectedPlantName)
                                                  echo ' · ' . h($selectedPlantName); ?>
                                        </div>
                                    </div>
                                        ...
                                        <table class="table align-middle mb-0">
                                            ...
                                            <tbody id="missing-tbody">
                                                <?php foreach ($missingAttendance as $person): ?>
                                                    ...
                                                    ?>
                                                    <tr data-row-type="missing" data-role="<?= h($mRoleSlug) ?>"
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            approvalForms.forEach(form => {
                form.addEventListener('submit', function (e) {
                    ...
                    if (submitBtn && submitBtn.value === 'reject') {
            ...
            /* =====================================================================
               MOBILE SWIPE CARDS – touch gesture handling
               Swipe LEFT  → Approve
               Swipe RIGHT → Reject
               Only active on mobile (< 768px) and for pending cards with checkout
               ===================================================================== */
            (function initSwipeCards() {
                if (window.innerWidth >= 768) return; // desktop – skip entirely

                const SWIPE_THRESHOLD = 100; // px needed to trigger action
                const MAX_ROTATION = 12;     // deg rotation at edge

                const csrfToken = '<?= h($csrfToken) ?>';

                document.querySelectorAll('.swipe-card-wrapper').forEach(wrapper => {
                    const card = wrapper.querySelector('.swipe-card');
                    const form = wrapper.querySelector('form[data-swipe-form]');
                    const canSwipe = wrapper.dataset.canSwipe === '1';
                    if (!card || !form || !canSwipe) return;
                    ...
                    function onMove(e) {
                        ...
                        const pct = Math.min(Math.abs(currentX) / 300, 1);
                        const rotation = (currentX / 300) * MAX_ROTATION;
                        card.style.transform = `translateX(${currentX}px) rotate(${rotation}deg)`;

                        // Add resistance if going too far
                        if (Math.abs(currentX) > 50) e.preventDefault();
                    }
                    ...
                    function onEnd() {
                        ...
                        if (Math.abs(currentX) >= SWIPE_THRESHOLD) {
                            // Determine action
                            const action = currentX < 0 ? 'approve' : 'reject';
                            const flyClass = currentX < 0 ? 'fly-left' : 'fly-right';

                            // Confirm before rejecting
                            if (action === 'reject') {
                                if (!confirm('Are you sure you want to reject this attendance record?')) {
                                    // User cancelled – snap card back
                            ...
                            // Trigger AJAX after animation
                            setTimeout(() => {
                                submitSwipeAction(wrapper, action, form);
                            }, 200);
                        } else {
                            // Snap back
                            card.style.transform = '';
                ...
                function submitSwipeAction(wrapper, action, form) {
                    const attendanceId = wrapper.dataset.attendanceId;
                    const formData = new FormData(form);
                    ...
                    fetch(window.location.href, {
                        ...
                        .then(data => {
                            ...
                            if (data && data.status === 'ok') {
                                const newStatus = data.newStatus || (action === 'approve' ? 'Approved' : 'Rejected');

                                // Show success overlay
                                const overlay = document.createElement('div');
                                overlay.className = 'sc-action-result action-' + action;
                                overlay.innerHTML = action === 'approve'
                                    ? '<i class="fas fa-check-circle"></i><span>Approved</span>'
                                    : '<i class="fas fa-times-circle"></i><span>Rejected</span>';
                                wrapper.appendChild(overlay);

                                // Reset card position with result showing
                                if (card) {
                                ...
                                wrapper.classList.remove('swiping-left', 'swiping-right');
                                wrapper.dataset.canSwipe = '0';
                                wrapper.dataset.status = newStatus.toLowerCase();
                                wrapper.dataset.statusLabel = newStatus.toLowerCase();

                                // Update badge
                                const badge = wrapper.querySelector('.sc-status-badge .badge');
                                if (badge) {
                                    badge.textContent = newStatus;
                                    badge.className = 'badge text-bg-' + mapStatusToBadge(newStatus);
                                }

                                // Also update the corresponding table row (for stats)
                                const tableRow = document.querySelector(`tr[data-row-type="approval"] input[value="${attendanceId}"]`);
                                if (tableRow) {
                                    const tr = tableRow.closest('tr');
                                    if (tr) {
                                        tr.dataset.status = newStatus.toLowerCase();
                                        tr.dataset.statusLabel = newStatus.toLowerCase();
                                        tr.dataset.statusSort = newStatus.toLowerCase();
                                    }
                                }

                                showTopAlert('success', data.message || `Attendance #${attendanceId} ${newStatus}.`);
                                recalcStatsAndTotals();

                                // Fade overlay after a moment
                                setTimeout(() => {
                                ...
                                wrapper.classList.remove('swiping-left', 'swiping-right');
                                showTopAlert('danger', (data && data.message) || 'Failed to update attendance.');
                            }
                        })
                        .catch(err => {
                            console.error(err);
                            ...
                            wrapper.classList.remove('swiping-left', 'swiping-right');
                            showTopAlert('danger', 'Network error – please try again.');
                        });
                }

                /* --- Mobile card filtering (mirror desktop logic) --- */
                function applyMobileCardFilters() {
                    const searchVal = (tableSearch?.value || '').trim().toLowerCase();
                    const statusVal = (statusSelect?.value || 'Pending').toLowerCase();

                    document.querySelectorAll('.swipe-card-wrapper').forEach(w => {
                        let show = true;
                        const wRole = (w.dataset.role || '');
                        const wName = (w.dataset.name || '');
                        const wPlant = (w.dataset.plant || '');
                        const wVehicle = (w.dataset.vehicle || '');
                        const wNotes = (w.dataset.notes || '');
                        const wStatus = (w.dataset.statusLabel || '');

                        if (roleFilter && roleFilter !== 'all' && wRole !== roleFilter) show = false;
                        if (statusVal !== 'all' && wStatus !== statusVal) show = false;
                        if (searchVal) {
                            const hay = [wName, wPlant, wVehicle, wNotes, wStatus].join(' ');
                            if (!hay.includes(searchVal)) show = false;
                        }
                ...
                // Hook into existing filter events
                const origApplyFilters = applyFilters;
                window._mobileCardFilter = applyMobileCardFilters;

                // Override applyFilters to also filter mobile cards
                const _superApplyFilters = applyFilters;
                // We can't reassign const, so we patch via event
                if (tableSearch) tableSearch.addEventListener('input', applyMobileCardFilters);

                // Initial
                applyMobileCardFilters();
            })();
        });
    </script>
    ...
    <div class="photo-lightbox-overlay" id="photoLightbox">
        <span class="photo-lightbox-close"><i class="fas fa-times"></i></span>
        <img src="" alt="Attendance Photo" id="photoLightboxImg">
        <div class="photo-lightbox-label" id="photoLightboxLabel"></div>
    </div>
    ...
    <script>
        (function initPhotoLightbox() {
            ...
            const lbLabel = document.getElementById('photoLightboxLabel');
            ...
            // Open lightbox on click of any [data-photo-url] link
            document.addEventListener('click', function (e) {
                var trigger = e.target.closest('[data-photo-url]');
                if (!trigger) return;
                e.preventDefault();
                e.stopPropagation();
                var url = trigger.dataset.photoUrl || trigger.getAttribute('href');
                if (!url) return;
                lbImg.src = url;
                var rawText = (trigger.textContent || '').trim().replace(/📸/g, '').trim();
                lbLabel.textContent = rawText ? rawText + ' Photo' : 'Photo';
