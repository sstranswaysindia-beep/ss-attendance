...
function admin_proxy_normalize_flag($value): bool
{
    if ($value === null) {
        return false;
    }
...
try {
    ...
    // Active drivers with proxy flag.
    $driverSql = "
    ...
    if ($driverResult) {
        while ($row = $driverResult->fetch_assoc()) {
            ...
            $users[] = [
                ...
                'plants' => $plantList,
                'proxyEnabled' => admin_proxy_normalize_flag($row['proxy_enabled'] ?? null),
    ...
    // Supervisors eligible for proxy actions.
    $supervisorSql = "
        SELECT u.id        AS user_id,
               u.full_name AS full_name,
               u.username  AS username,
