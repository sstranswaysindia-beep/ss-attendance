...
function geofenceLocationBufferMeters(?array $location): float {
    if ($location === null) {
        return 0.0;
    }
    $accuracy = 0.0;
    if (isset($location['accuracy']) && is_numeric($location['accuracy'])) {
        $accuracy = max(0.0, (float) $location['accuracy']);
    }
    $buffer = 20.0;
    if ($accuracy > 0.0) {
...
function geofenceDistancePointToSegmentMeters(
    float $lat,
    float $lng,
    float $lat1,
    float $lng1,
    ...
    $lat0 = deg2rad($lat);
    $toXY = static function (float $pointLat, float $pointLng) use ($lat0, $lng, $earthRadius): array {
        $x = deg2rad($pointLng - $lng) * cos($lat0) * $earthRadius;
        $y = deg2rad($pointLat - $lat) * $earthRadius;
        return [$x, $y];
    };
    [$x1, $y1] = $toXY($lat1, $lng1);
    [$x2, $y2] = $toXY($lat2, $lng2);
    $dx = $x2 - $x1;
    $dy = $y2 - $y1;
 ...
 */
function geofenceDistanceToPolygonMeters(float $lat, float $lng, array $points): float {
    ...
    for ($i = 0, $j = $numPoints - 1; $i < $numPoints; $j = $i++) {
        $start = $points[$j];
        $end = $points[$i];
        $distance = geofenceDistancePointToSegmentMeters(
            $lat,
            $lng,
            $start['lat'],
            $start['lng'],
...
function geofenceEvaluate(
    ...
    $inside = false;
    $buffer = geofenceLocationBufferMeters($location);
    ...
    foreach ($geofences as $geofence) {
        ...
        if ($type === 'circle') {
            ...
            if ($buffer > 0.0) {
                $edgeDistance = geofenceDistanceToPolygonMeters(
                    $lat,
                    $lng,
                    $geofence['polygon_points']
                );
                if ($edgeDistance <= $buffer) {
                    $inside = true;
                    break;
                }
            }
